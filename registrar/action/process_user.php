    ...
    try {
        switch ($action) {
            case 'add':
                ...
                $stmt = $conn->prepare("INSERT INTO registrar (name, email, password) VALUES (?, ?, ?)");
            ...
            case 'edit':
                ...
                if (!empty($password)) {
                    $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                    $stmt = $conn->prepare("UPDATE registrar SET name = ?, email = ?, password = ? WHERE id = ?");
                    $stmt->bind_param("sssi", $name, $email, $hashed_password, $id);
                } else {
                    $stmt = $conn->prepare("UPDATE registrar SET name = ?, email = ? WHERE id = ?");
