...
//for verified email
    ...
    if(isset($_POST['check-email'])){
        ...
        $check_email = "SELECT * FROM ssc WHERE email='$email'";
        $run_sql = mysqli_query($con, $check_email);
        if(mysqli_num_rows($run_sql) > 0){
            $code = rand(100000, 999999);
            $insert_code = "UPDATE ssc SET OTP = $code WHERE email = '$email'";
...
//         $data_check = mysqli_query($con, $insert_data);
    ...
    if(isset($_POST['check-reset-otp'])){
        ...
        $check_code = "SELECT * FROM ssc WHERE OTP = $otp_code";
    ...
    if(isset($_POST['change-password'])){
        ...
        if($password !== $cpassword){
            ...
            $update_pass = "UPDATE ssc SET OTP = $code, password = '$encpass' WHERE email = '$email'";
